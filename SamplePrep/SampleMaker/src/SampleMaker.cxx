...
int main(int argc, char *argv[])
{
    ...
    float calc_topoetcone20;
    unnormedTree->Branch("calc_topoetcone20", &calc_topoetcone20, "calc_topoetcone20/F");
    float calc_topoetcone30;
    unnormedTree->Branch("calc_topoetcone20", &calc_topoetcone30, "calc_topoetcone30/F");
    float calc_topoetcone40;
    unnormedTree->Branch("calc_topoetcone40", &calc_topoetcone40, "calc_topoetcone40/F");
    ...
    float PLT;
    unnormedTree->Branch("baseline_PLT", &PLT, "baseline_PLT/F");
    ...
    auto process_lepton = [&](const xAOD::IParticle *lepton, const xAOD::Vertex *primary_vertex, const vector<const xAOD::TrackParticle *> &filtered_tracks, vector<const xAOD::CaloCluster *> &filtered_calo_clusters, vector<const xAOD::Jet *> &filtered_jets, bool is_electron) {
        ...
        //calc_prcone and calc_ptvarcone
        if (is_electron)
        {
            calc_ptcone20 = 0;
            calc_ptcone30 = 0;
        ...
        if (is_electron)
        {
            ...
            const xAOD::CaloCluster *egclus = lepton->caloCluster();
            for (const auto &clus : *filtered_calo_clusters)
            {
                if (clus->e() < 0)
                    continue;
                if (egclus->p4().DeltaR(clus->p4()) < 0.2)
                    calc_etcone20 += clus->et();
                if (egclus->p4().DeltaR(clus->p4()) < 0.3)
                    calc_etcone30 += clus->et();
                if (egclus->p4().DeltaR(clus->p4()) < 0.4)
                    calc_etcone40 += clus->et();
            }
            //calc subtraction 5times7 cell from CaloIsolationTool::correctIsolationEnergy_Eeg57
            float coreV = 0;
            const xAOD::CaloCluster *cleg = lepton->caloCluster();
            ...
            calc_etcone20 -= coreV; calc_etcone30 -= coreV; calc_etcone30 -= coreV
        }
        /*
...
*/
    ...
    for (entry_n = 0; entry_n < entries; ++entry_n)
    {
        ...
        for (auto electron_info : filtered_electrons)
        {
            ...
            truth_type = electron_info.second;
            pdgID = 11;
            if (!process_lepton(electron, primary_vertex, filtered_tracks, filtered_calo_clusters, filtered_jets, true))
                continue;
        ...
        for (auto muon_info : filtered_muons)
        {
            ...
            truth_type = muon_info.second;
            pdgID = 13;
            if (!process_lepton(muon, primary_vertex, filtered_tracks, filtered_calo_clusters, filtered_jets, false))
                continue;
