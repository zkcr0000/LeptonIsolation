...
# Install the project:
DESTDIR=${DESTDIR}/usr/WorkDir/1.0 \
    make -k install/fast

# Remove the .dbg files from being packaged, in case we are in RelWithDebInfo
# build mode:
rm -f ${DESTDIR}/usr/WorkDir/1.0/InstallArea/x86_64-centos7-gcc8-opt/bin/*.dbg
rm -f ${DESTDIR}/usr/WorkDir/1.0/InstallArea/x86_64-centos7-gcc8-opt/lib/*.dbg
